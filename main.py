...
def helper(dis):
    desc = " ".join(description.loc[description['Disease'] == dis, 'Description'].dropna())
    pre = precautions.loc[precautions['Disease'] == dis, ['Precaution_1', 'Precaution_2', 'Precaution_3',
                                                          'Precaution_4']].dropna().values.flatten().tolist()
...
# Symptoms dictionary (Truncated for brevity)
...
def predict():
    ...
    if not user_symptoms:
        return render_template('index.html', message="No valid symptoms recognized. Please check your input.")
